...
class MineManager(metaclass=Singleton):
    ...
    def __init__(self, config_path):
        ...
        self.commit_miner, self.dev_miner, \
            self.file_miner, self.method_miner,\
                self.branch_miner = None, None, None, None, None
        try:
            if not config_path:
    ...
    def connect(self):
        ...
        try:
            ...
            # Connect using Bolt protocol - port 7687 is for Bolt, not HTTP
            self.graph = Graph(f"bolt://{self.config.ct.db_url}:{self.config.ct.port}",
    ...
    def init_miners(self):
        ...
        try:
            ...
            self.branch_miner = miners.BranchMiner(
                graph=self.graph,
